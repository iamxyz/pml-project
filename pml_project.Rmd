...
Notes: The variables:"X","user_name","raw_timestamp_part_1","raw_timestamp_part_2","cvtd_timestamp","new_window","num_window" are removed from the predictors because of they are irrelevant to prediction of classes of activity. So We get two final datasets: training_set for model training and validation, testing_set for model testing. 
```{r}
...
We give first priority to accuracy of the models in our work, so the RF model is choosed and trained on datasets by cross validation, then the best fitted RF model is selected for prediction on test dataset. 
...
Based on the five accuracy values of fitted RF model stored before, we choose the fitted model with lowest OSE and its prediction result on testing_set. so the predictions for activity classes of the 20 test cases is:
```{r}
